...
do
   ...
   do
      ...
      fmt -1 sorteo.dat | awk -v verdad=${nmpu[$j]} '{if(NR<=verdad)print $1 " =1";else print $1 " = 0"}' |sort -n >reglas.dat
  ...
  i.maxlik group=$grupo subgroup=$subgrupo sigfile=$mvtr$i class=${mvt}${i}_$etiqueta
